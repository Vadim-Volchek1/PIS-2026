"""
gRPC client for bug tracker request service.
"""
import os
import sys
...
def create_request(stub):
    ...
    response = stub.CreateRequest(
        request_service_pb2.CreateRequestRequest(
            reporter_id="qa.petrov",
            project_id="PIS-WEB",
            title="500 при загрузке вложения",
            description="Ошибка на /api/attachments",
            priority="HIGH",
        )
    )
    print(f"Created: {response.request_id}")
...
def get_request(stub, request_id):
    ...
    try:
        response = stub.GetRequest(request_service_pb2.GetRequestRequest(request_id=request_id))
        if response.found:
            bug = response.request
            print(f"Bug: {bug.request_id}")
            print(f"Project: {bug.project_id}")
            print(f"Reporter: {bug.reporter_id}")
...
def list_requests(stub, status_filter=""):
    print(f"\n=== ListRequests(filter={status_filter or 'ALL'}) ===")
    response = stub.ListRequests(
        request_service_pb2.ListRequestsRequest(status_filter=status_filter, limit=20)
    )
    print(f"Found: {response.total_count}")
    for row in response.requests:
        print(f" - {row.request_id} [{row.status}] {row.title}")


def activate_request(stub, request_id):
    print(f"\n=== ActivateRequest({request_id}) ===")
    response = stub.ActivateRequest(
        request_service_pb2.ActivateRequestRequest(request_id=request_id)
    )
    print(f"Success: {response.success}, Error: {response.error_message}")


def stream_active_requests(stub):
    print("\n=== StreamActiveRequests ===")
    print("Streaming active bugs (Ctrl+C to stop)...")
    try:
        for row in stub.StreamActiveRequests(request_service_pb2.StreamActiveRequestsRequest()):
            print(f" -> {row.request_id} [{row.status}] {row.title}")
...
def run():
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        bug_id = create_request(stub)
        get_request(stub, bug_id)
        activate_request(stub, bug_id)
        list_requests(stub)
        list_requests(stub, "ACTIVE")
