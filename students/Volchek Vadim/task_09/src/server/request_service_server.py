"""
gRPC server for bug tracker request service.
"""
import os
import sys
import time
...
class RequestServiceServicer(request_service_pb2_grpc.RequestServiceServicer):
    ...
    def _seed_data(self):
        now = int(time.time())
        seed = [
            ("qa.petrov", "PIS-WEB", "Не работает логин", "500 на /api/auth", "HIGH", "ACTIVE"),
            ("qa.sidorov", "PIS-API", "Ошибка валидации", "422 вместо 400", "MEDIUM", "DRAFT"),
            ("qa.ivanov", "PIS-WEB", "Падает экспорт", "traceback в /export", "CRITICAL", "COMPLETED"),
        ]
        for reporter_id, project_id, title, description, priority, status in seed:
            request_id = f"BUG-2026-{self.counter:04d}"
            self.counter += 1
            activated_at = now - 1800 if status in ("ACTIVE", "COMPLETED") else 0
            completed_at = now - 300 if status == "COMPLETED" else 0
            self.requests[request_id] = request_service_pb2.Bug(
                ...
                priority=priority,
                status=status,
                assigned_group_id="TEAM-1" if status != "DRAFT" else "",
                created_at=now - 3600,
    ...
    def GetRequest(self, request, context):
        bug = self.requests.get(request.request_id)
        if bug:
            return request_service_pb2.GetRequestResponse(request=bug, found=True)

        context.set_code(grpc.StatusCode.NOT_FOUND)
        context.set_details(f"Bug {request.request_id} not found")
        return request_service_pb2.GetRequestResponse(found=False)

    def ListRequests(self, request, context):
        status_filter = request.status_filter.strip()
        rows = [
            row
        ...
        rows = rows[:limit]
        return request_service_pb2.ListRequestsResponse(requests=rows, total_count=len(rows))
    ...
    def ActivateRequest(self, request, context):
        ...
        if not bug:
            ...
            return request_service_pb2.ActivateRequestResponse(
                success=False,
                error_message="Bug not found",
            )
        if bug.status != "DRAFT":
            return request_service_pb2.ActivateRequestResponse(
                success=False,
                error_message=f"Cannot activate bug with status {bug.status}",
            )

        bug.status = "ACTIVE"
        bug.activated_at = int(time.time())
        self.requests[request.request_id] = bug
        return request_service_pb2.ActivateRequestResponse(success=True, error_message="")

    def StreamActiveRequests(self, request, context):
        while context.is_active():
            active = [row for row in self.requests.values() if row.status == "ACTIVE"]
            for row in active:
                yield row
                time.sleep(0.5)
            time.sleep(2)
