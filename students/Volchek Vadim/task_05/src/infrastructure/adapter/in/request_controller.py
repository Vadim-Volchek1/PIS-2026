"""
BugController: FastAPI REST endpoints for bug tracker.
"""
...
class CreateBugRequest(BaseModel):
    reporter_id: str = Field(..., example="qa.petrov")
    project_id: str = Field(..., example="PIS-WEB")
    title: str = Field(..., example="500 при загрузке вложения")
    description: str = Field(..., example="Ошибка на /api/attachments")
...
class AssignBugRequest(BaseModel):
    assignee_id: str = Field(..., example="dev.ivanov")


@router.post("", status_code=status.HTTP_201_CREATED, response_model=CreateBugResponse)
def create_bug(_: CreateBugRequest):
    # TODO: connect CreateRequestHandler from Application layer
    return CreateBugResponse(bug_id="BUG-2026-0001")


@router.get("/{bug_id}")
def get_bug(bug_id: str):
    # TODO: connect GetRequestByIdHandler from Application layer
    if bug_id != "BUG-2026-0001":
        raise HTTPException(status_code=404, detail=f"Bug {bug_id} не найден")
    return {
        "request_id": "BUG-2026-0001",
        "reporter_id": "qa.petrov",
        "project_id": "PIS-WEB",
        "title": "500 при загрузке вложения",
        "priority": "HIGH",
        "status": "ACTIVE",
    }


@router.post("/{bug_id}/assign", status_code=status.HTTP_200_OK)
def assign_bug(bug_id: str, _: AssignBugRequest):
    # TODO: connect AssignGroupToRequestHandler from Application layer
    return {"status": "assigned", "request_id": bug_id}


@router.get("", response_model=List[dict])
def list_bugs():
    # TODO: connect ListActiveRequestsHandler from Application layer
    return [{"request_id": "BUG-2026-0001", "status": "ACTIVE", "priority": "HIGH"}]
"""
