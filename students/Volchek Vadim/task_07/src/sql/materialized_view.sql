-- ============================================
-- CQRS Read Model for bug tracker
-- ============================================

CREATE MATERIALIZED VIEW bug_view AS
SELECT
    b.request_id,
    b.status,
    b.project_id,
    ...
    b.reporter_id,
    b.assigned_group_id AS assignee_id,
    pr.name AS priority,
    b.title,
    b.created_at,
    b.activated_at,
    b.completed_at,
    CASE
        WHEN b.activated_at IS NOT NULL AND b.completed_at IS NOT NULL
            THEN EXTRACT(EPOCH FROM (b.completed_at - b.activated_at)) / 60
        ELSE NULL
...
LEFT JOIN bug_priorities pr ON pr.request_id_fk = b.id;

CREATE UNIQUE INDEX idx_bug_view_id ON bug_view (request_id);
CREATE INDEX idx_bug_view_status ON bug_view (status);
CREATE INDEX idx_bug_view_project ON bug_view (project_id);
CREATE INDEX idx_bug_view_assignee ON bug_view (assignee_id);

-- Manual refresh
REFRESH MATERIALIZED VIEW bug_view;
-- =====================================================
