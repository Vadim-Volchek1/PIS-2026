"""
Integration tests for bug repository mapping with PostgreSQL.
"""
import pytest
...
class TestBugRepositoryIntegration:
    def test_should_save_new_bug_row(self, db_session):
        bug_row = RequestORM(
        ...
        found = db_session.query(RequestORM).filter_by(request_id="BUG-2026-0001").first()
        assert found is not None
        assert found.reporter_id == "qa.petrov"
        assert found.project_id == "PIS-WEB"

    def test_should_find_by_bug_id(self, db_session):
        bug_row = RequestORM(
            request_id="BUG-2026-0002",
            reporter_id="qa.sidorov",
            project_id="PIS-API",
            title="Падает endpoint",
            description="500 на /api/attachments",
            status="ACTIVE",
        )
        db_session.add(bug_row)
        db_session.commit()

        found = db_session.query(RequestORM).filter_by(request_id="BUG-2026-0002").first()
        assert found is not None
...
class TestBugRepositoryMapping:
    def test_should_save_new_bug_record(self, db_session):
        orm_bug = RequestORM(
            request_id="BUG-2026-0001",
            reporter_id="qa.petrov",
            project_id="PIS-WEB",
