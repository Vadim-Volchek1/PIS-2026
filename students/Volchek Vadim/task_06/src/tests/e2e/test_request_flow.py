"""
E2E tests for full bug API flow.
"""
...
class TestBugFlowE2E:
    def test_create_and_get_bug(self):
        ...
        assert get_response.json()["request_id"] == bug_id

    def test_assign_bug_endpoint(self):
        client = TestClient(app)
        response = client.post(
            "/api/bugs/BUG-2026-0001/assign",
            json={"assignee_id": "dev.ivanov"},
        )
...
class TestBugFlowE2E:
    def test_create_and_get_bug(self):
        ...
        bug_id = create_response.json().get("bug_id", "BUG-2026-0001")
        get_response = client.get(f"/api/bugs/{bug_id}")
        assert get_response.status_code == 200
        assert get_response.json()["request_id"].startswith("BUG-")
"""
